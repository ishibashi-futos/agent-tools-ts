...
describe("factory.ts (createSecureTool)", () => {
  ...
  const mockDomainFn = async (path: string, data: string) => `Processed ${path}`;

  it("Security Layer: ポリシー拒否時に status: 'denied' を返すこと", async () => {
    const deniedContext = { ...defaultContext, policy: { tools: { "test_tool": "deny" }, defaultPolicy: "deny" } };
    const secureTool = createSecureTool({ name: "test_tool", isWriteOp: true }, mockDomainFn);

    const result = await secureTool(deniedContext as any, "src/index.ts", "data");
  ...
  it("Sandbox FS Layer: Read-onlyモードでの書き込み時に status: 'denied' を返すこと", async () => {
    ...
    const result = await secureTool(readOnlyContext as any, "src/index.ts", "data");
  ...
  it("Sandbox Path Layer: ワークスペース外アクセス時に status: 'denied' を返すこと", async () => {
    const secureTool = createSecureTool({ name: "test_tool", isWriteOp: false }, mockDomainFn);
